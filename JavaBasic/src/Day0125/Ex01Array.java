package Day0125;
// 배열(Array)
// 배열이란 '지정된 크기'만큼의 '똑같은 데이터타입'이 모여있는 하나의 자료형을
// 배열이라고 한다.
...
// 예를 들어 우리가 int 배열 intArray 의 2번재에 값을 할당할 때에는
...
// 참조형 데이터 타입의 배열은 모든 위치를 null로 초기화한다.
// null이란 특수한 "상태"로서, 참조형 데이터타입이 메모리 주소값을 부여가 되었지만
// 해당 메모리 주소값을 참고해서 들어가면 아무것도 없는 상태를 null이라고 하고
// null 인 상태에서는 새로운 값을 할당하는 것 외에는 아무것도 할 수 없다.

// NullPointException => NPE

// 또한 배열의 존재하지 않는 위치를 접근할려고 한다면
// ArrayIndexOutOfBoundException, 즉 잘못된 위치 접근 에러가 발생하게 된다.
// 위치를 지정하는 "위치 번호"는 우리가 index라고 부른다.
// 배열의 인덱스는 0부터 해당 (배열 크기 - 1) 까지 이다.
...
public class Ex01Array {
    public static void main(String[] args) {
        ...
        // 참조형 데이터타입의 배열인 srtingArray의 값을 출력해보자
        ...
        // 그렇다면 null 은 어떻게 비교해야 할까?
        // null 은 "상태"이므로 비교연산자를 사용하여 비교해야 한다.
